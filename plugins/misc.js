...
let a = await getBuffer(`https://citel-x.herokuapp.com/ttp/${text}`)
 ...
 cmd({
             pattern: "readmessage",
             desc: "activates and deactivates read message.\nuse buttons to toggle.",
             category: "misc",
             filename: __filename
         },
         async(Void, citel, text,{ isCreator }) => {
             if (!isCreator) return citel.reply(tlang().owner)
             const { readmessagethama } = require('../lib/');
             switch (text.split(" ")[0]) {
                 case "on":
                     {
                      let readmessagee = await readmessagethama.findOne({ id: 'readmessagethama' })
                     if (!readmessagee) {
                         await new readmessagethama({ id: 'readmessagethama', worktype: "true" }).save()
                         return citel.reply('Read Message activated successfully.')
                     } else {
                         if (readmessagee.worktype == "true") return citel.reply("Chatbot was already enabled.")
                         await readmessagethama.updateOne({ id: 'readmessagethama' }, { worktype: "true" })
                         citel.reply('Enabled Read Message successfully.')
                         return
                     }      
                     }
                     break
                 case "off":
                     {
                      let readmessagee= await readmessagethama.findOne({ id: 'readmessagethama' })
                     if (!readmessagee) {
                         await new readmessagethama({ id: 'readmessagethama', worktype: "false" }).save()
                         return citel.reply('Read Message deactivated successfully.')
                     } else {
                         if (readmessagee.worktype == "false") return citel.reply("Chatbot was already disabled.")
                         await readmessagethama.updateOne({ id: 'readmessagethama' }, { worktype: "false" })
                         citel.reply('Disabled Read Message successfully.')
                         return
                     }
                     }
                     break
                 default:
                     {
                         let buttons = [{
                                 buttonId: `${prefix}Read Message on`,
                                 buttonText: {
                                     displayText: "Turn On",
                                 },
                                 type: 1,
                             ...
                             {
                                 buttonId: `${prefix}Read Message off`,
                                 buttonText: {
                                     displayText: "Turn Off",
                                 },
                                 type: 1,
                         ...
                         let readmessagee= await readmessagethama.findOne({ id: 'readmessagethama' })
                         await Void.sendButtonText(citel.chat, buttons, `Chatbot Status: ${readmessagee.worktype} `, 'Secktor-Md', citel);
                        citel.reply(`Read Message Status: ${readmessagee.worktype} \n*Use:* ${prefix}readmessage on\n${prefix}readmessage off`)
                        }
             }
